...
router.get("/", async (req, res) => {
    ...
    try {
        const user = userId
            ? await User.findById(userId)
            : await User.findOne({ usersId: usersId });
        const { password, updatedAt, ...other } = user._doc;
        res.status(200).json(other);
    } catch (err) {
        res.status(500).json(err);
...
router.get("/allusers", async (req, res) => {
    try {
        const usersList = await User.find().select('username profilePicture usersId');
        res.send(usersList);
    } catch (err) {
...
router.get("/friends/:userId", async (req, res) => {
    try {
        ...
        let friendList = [];
        friends.map(friend => {
            const { _id, username, usersId, profilePicture } = friend;
            friendList.push({ _id, username, usersId, profilePicture });
        });
